...
const Disaster_Relief_Alliance = () => {
      ...
      const fetchRating = async () => {
        try {
          const response = await axios.post('http://localhost:5000/extract_rating_out', {
            input_string: "Disaster Relief Alliance"
          });
        ...
        const fetchOrganization = async () => {
            try {
                const response = await fetch('http://localhost:5000/organization/name', {
                    ...
                    body: JSON.stringify({ name: "Disaster Relief Alliance" }) // Provide the organization name here
                });
                if (!response.ok) {
...
export default Disaster_Relief_Alliance;
