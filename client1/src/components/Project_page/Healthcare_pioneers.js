      ...
      const fetchRating = async () => {
        try {
          const response = await axios.post('http://localhost:5000/extract_rating_out', {
            input_string: 'HealthCare Pioneers'
          });
        ...
        const fetchOrganization = async () => {
            try {
                const response = await fetch('http://localhost:5000/organization/name', {
                    ...
                    body: JSON.stringify({ name: 'HealthCare Pioneers' }) // Provide the organization name here
                });
                if (!response.ok) {
...
export default HealthCare_Pioneers;
